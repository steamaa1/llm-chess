...
export type LlmTurn = { move: Move; commentary: string; provider: string; model: string; durationMs: number };
...
export async function requestLlmMove(config: LlmConfig, side: Side, moves: CompactMove[]): Promise<LlmTurn> {
  let response: Response;
  try { response = await fetch('/api/llm/move', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ config, side, moves }) }); }
