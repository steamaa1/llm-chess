...
export type LlmTurn = { move?: Move; commentary?: string; undo?: boolean; undoReason?: string; rawOutput?: string; provider: string; model: string; durationMs: number; promptTokens: number; completionTokens: number };
...
export class LlmRequestError extends Error { constructor(public code: string, message: string, public modelOutput?: string) { super(message); this.name = 'LlmRequestError'; } }
...
export async function requestLlmMove(config: LlmConfig, side: Side, moves: CompactMove[], context: LlmContext): Promise<LlmTurn> {
  ...
  if (!response.ok) {
    const error = body && typeof body === 'object' && 'error' in body ? (body as { error?: { code?: string; message?: string; modelOutput?: string } }).error : undefined;
    throw new LlmRequestError(error?.code ?? 'UNKNOWN_ERROR', error?.message ?? '模型请求失败，棋局未改变。', error?.modelOutput);
  }
