...
type AnalysisEvent = { id: string; side: Side; move?: string; commentary?: string; status: 'requesting' | 'success' | 'error'; detail: string; at: string; promptTokens?: number; completionTokens?: number };
...
export function App() {
  ...
  return <main className="app-shell">
    ...
    <section id="main-content" className="game-layout" aria-label="对局区">
      ...
      <section className="error-spot">{appError && <div className="error-banner" role="alert"><strong>{appError.code}</strong><span>{appError.message}</span><button type="button" onClick={() => { setAppError(null); setPaused(false); }}>关闭并重试</button></div>}</section>
      <section className="board-panel" aria-label="中国象棋棋盘"><div className="board-frame"><div className="board" role="application" aria-label="标准中国象棋开局，红方在下" onDragOver={(event) => event.preventDefault()} onDrop={handleBoardDrop}><BoardLines /><div className="river" aria-hidden="true"><span>楚 河</span><span>漢 界</span></div>>红方在下 · 标准开局 · 点击棋子查看合法落点</p></section>
